...
def validate_name(name: str, *, allow_default: bool = True) -> str:
    ...
    if name == DEFAULT_TAG and not allow_default:
        raise ValueError("The name 'default' is reserved for the existing Tag")
    return name
...
def resolve(installation_root: Path, tag_id: str = DEFAULT_TAG, *, require_exists: bool = True) -> InstanceContext:
    ...
    if tag_id != DEFAULT_TAG and require_exists:
        ...
        try:
            ...
            raise ValueError(f"Tag '{tag_id}' has malformed instance metadata") from None
        if record.get("schema_version") != SCHEMA_VERSION or record.get("id") != tag_id:
            raise ValueError(f"Tag '{tag_id}' has malformed instance metadata")
...
def discover(installation_root: Path) -> list[dict[str, object]]:
    ...
    result: list[dict[str, object]] = [
        {"id": DEFAULT_TAG, "home": str(root), "valid": True, "error": None}
    ]
    ...
    for entry in entries:
        ...
        try:
            validate_name(entry.name, allow_default=False)
