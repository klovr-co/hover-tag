...
| Connect Slack | `slack login` supplies a slash-command authorization ticket; the user approves in Slack and returns a challenge code. | Show the real handoff, not a promised automatic browser OAuth redirect. Never place tickets/codes in logs or agent chat. |
