...
| Direct message receives no reply | DM invocation was disabled or the Slack app lacks its DM event configuration | Ensure `OPENTAG_SLACK_DM_ENABLED` is not `0`, reinstall the current app manifest, restart Tag, and confirm the sender is in `SLACK_ALLOWED_USER_IDS`. |
