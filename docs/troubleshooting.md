...
| Mention receives no reply | Slack did not emit an event or the bridge rejected the channel | Confirm Socket Mode is connected, mention `@Tag` (or the customized app name) from a human account, and verify the channel is in `SLACK_CHANNEL_IDS`. |
